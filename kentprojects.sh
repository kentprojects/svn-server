...
function CreateRepository
{
	...
	if [ -d "$SVNBASE" ]; then
		echo "A repository already exists at $URL"
		return 2
	fi
	if [ -d "$TRACBASE" ]; then
		echo "A repository already exists at $URL"
...
# @param string $2 The user name
# @param string $3 The user's password
# @return void
#
...
# @return void
#
function RemoveUserFromRepository
{
	if [ -z "$1" ]; then
		echo "Please supply a repository name to RemoveUserFromRepository"
		return 1;
	fi
	if [ -z "$2" ]; then
		echo "Please supply a user name to RemoveUserFromRepository"
		return 1;
	fi
	...
	if [ -d "$SVNBASE" ]; then
		sed -i "/^$2 =/d" "$SVNBASE/conf/passwd"
		sudo chmod 775 -R "$SVNBASE"
	fi
	if [ -d "$TRACBASE" ]; then
		sed -i "/^$2:/d" "$TRACBASE/conf/passwd"
		sudo -u trac trac-admin "$TRACBASE" permission remove "$2" "*"
		sudo -u trac trac-admin "$TRACBASE" deploy "$TRACBASE/deploy"
		sudo chmod 775 -R "$TRACBASE"
		sudo chmod 777 -R "$TRACBASE/db"
	fi
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
	...
	case "$FUNC" in
		...
		"RemoveUserFromRepository") RemoveUserFromRepository $@ ;;
